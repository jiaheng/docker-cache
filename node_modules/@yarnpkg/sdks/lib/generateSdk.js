...
const getDisplayName = (name) => (0, compat_1.startCase)(name).split(` `).map(word => (0, compat_1.capitalize)(word)).join(` `);
...
class Wrapper {
    constructor(name, { pnpApi, target, manifestOverrides = {} }) {
        this.paths = new Map();
        this.name = name;
        this.pnpApi = pnpApi;
        this.target = target;
    ...
    loadManifest() {
        ...
        const manifestPath = fslib_1.npath.join(pkgInformation.packageLocation, fslib_1.Filename.manifest);
        const manifest = (0, dynamicRequire_1.dynamicRequire)(manifestPath);
        return {
            name: this.name,
    ...
    async writeDefaults() {
        if (this.manifest.main)
            await this.writeFile(fslib_1.ppath.normalize(this.manifest.main), { requirePath: fslib_1.PortablePath.dot });
    ...
    async writePackageBinaries() {
        if (typeof this.manifest.bin === `string`) {
            await this.writeBinary(fslib_1.ppath.normalize(this.manifest.bin));
        }
        else {
            for (const relPackagePath of Object.values(this.manifest.bin)) {
                await this.writeBinary(fslib_1.ppath.normalize(relPackagePath));
            }
        }
    }
    async writePackageExports(packageExports = this.manifest.exports, requirePath = fslib_1.PortablePath.dot) {
        if (typeof packageExports === `string`) {
            if (!packageExports.includes(`*`)) {
                await this.writeFile(fslib_1.ppath.normalize(packageExports), { requirePath });
            }
        }
            ...
            await Promise.all(Object.entries(packageExports).map(async ([key, value]) => {
                if (key.startsWith(`.`)) {
                    await this.writePackageExports(value, key);
                }
                else {
    ...
    async writeManifest() {
        ...
        await fslib_1.xfs.writeJsonPromise(absWrapperPath, this.manifest);
        this.paths.set(fslib_1.Filename.manifest, relProjectPath);
    }
    ...
    async writeFile(relPackagePath, options = {}) {
        ...
        const moduleReqPath = fslib_1.ppath.join(this.name, options.requirePath ?? relPackagePath);
        const wrapperScript = TEMPLATE(relPnpApiPath, moduleReqPath, options);
        await fslib_1.xfs.mkdirPromise(fslib_1.ppath.dirname(absWrapperPath), { recursive: true });
        await fslib_1.xfs.writeFilePromise(absWrapperPath, wrapperScript, { mode: options.mode });
